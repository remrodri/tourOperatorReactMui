...
const TourPackageInfo: React.FC<TourPackageInfoProps> = ({ tourPackage }) => {
  return (
    <Box
    sx={{p:1 ,display:'flex',flexDirection:'row',gap:1,flexWrap:'wrap',height:'100%'}}
    >
      <Typography variant="h5" gutterBottom sx={{display:'flex',justifyContent:'center', width:'100%',height:'10%'}}>
        Paquete turistico
      </Typography>
      ...
      {tourPackage?.itinerary &&
        ...
        tourPackage.itinerary.days.map((day, index) => (
          <Box key={index}>
            <Typography variant="body1" gutterBottom>Dia: {day.dayNumber}</Typography>
            {day.activities &&
